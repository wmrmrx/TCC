...
\title{Versão Rainbow do Teorema de Dirac: uma abordagem algorítmica}
\author{
...
\begin{frame}[fragile]\centering
  ...
  \begin{columns}[T]
    ...
    \begin{block}{Conceitos básicos}
        ...
        $\cdot$ Um grafo simples $G$ é um grafo não direcionado sem laços e sem arestas múltiplas, onde $V(G)$ e $E(G)$ são definidos, respectivamente, o conjunto de vértices e o conjunto de arestas desse grafo.\\
        $\cdot$ Um \textbf{circuito hamiltoniano} de $G$ é um circuito que visita cada vértice de $G$ exatamente uma vez.\\
        $\cdot$ Definimos $\deg(v)$ como o número de vizinhos de um vértice de $v$ em $V(G)$, e $\delta(G) \coloneqq \min\{ \deg(v) : v \in V(G) \}$ como o grau mínimo de um vértice em $G$.\\
        \vspace{0.5em}
    ...
    \begin{block}{Técnicas utilizadas}
      ...
      \begin{center}
        ...
        \captionof{figure}{Construção de um circuito de tamanho $k+1$ a partir de um caminho de tamanho $k$.}
      \end{center}
      ...
      \begin{center}
        ...
        \captionof{figure}{Construção de um caminho de tamanho $k+1$ a partir de um circuito de tamanho $k$.}
      \end{center}
    ...
    \begin{block}{Desafios enfrentados}
      ...
      Esta é a parte mais difícil do algoritmo. Quase metade do código serve para lidar com esse caso, 
      pois são muitos detalhes a serem considerados.

      \vspace{0.6em}
      \textbf{Implementação eficiente:}

      A complexidade do algoritmo é da ordem de $\text{O}(n^3)$, em que $n$ é a ordem de $G$. Essa complexidade é ótima, pois existem $\text{O}(n^3)$ arestas que devem ser consideradas no input.
