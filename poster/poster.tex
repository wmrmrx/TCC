...
\posterbox[adjusted title = Dirac's Theorem and Basic Definitions]
    ...
    \begin{itemize}
        \item \textbf{Dirac's Theorem (1952):} If a simple graph $G = (V, E)$ with $n$ vertices has minimum degree $\delta(G) \geq \frac{n}{2}$, then $G$ contains a Hamiltonian cycle.
       
        \item \textbf{Key Definitions:}
        \begin{itemize}
	    \item A \textcolor{imered}{Hamiltonian cycle} of $G$ visits every vertex of $V$ exactly once.
	    \item $\delta(G)$ is the minimum degree of a vertex in $V$
        \end{itemize}
    \end{itemize}
}
