...
The total number of vertices is denoted by $n$, and the graph that is the union of $n$ graphs is represented by $G = G_1 \cup G_2 \cup \dots \cup G_n$, where each graph $G_i$ 
...
a path of length $\ell+1$. In \ref{fig:flowchart}, a $L$-Path is a path of length $L$ and a $L$-cycle is a cycle of length $L$.
