...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Function{Extend\_Cycle}{$G, C$}
            ...
            \State $y \gets \text{primeiro } i \in \{0, \dots, n-1\} \text{ tal que } \textbf{not } vertices\_in\_cycle[i]$
            \State $cy \gets \text{primeiro } i \in \{0, \dots, n-1\} \text{ tal que } \textbf{not } colors\_in\_cycle[i]$
...
$$
A(D) = \bigcup_{i \in \{0,..., n - 2\}} \{\{x_i, y\} : y \neq x_{i + 1}, edge(x_i, y, C.edge[i].color) \in E(G)\}.
$$
...
$|A(D)| \geq (n-1)(\frac{n}{2} - 1)$. Let's define the following auxiliary function
$find\_adjacency(u, color, vertex\_positions)$:

\begin{itemize}
    \item $u$ is a vertex of $G$,
    \item $color$ is a color of $G$,
    \item $vertex\_positions$ is an array of size $n$ where $vertex\_positions[i]$ is the 
    current position of vertex $i$. This is useful when we need to change the position
    of the vertex in the cycle.
\end{itemize}

This function gives the indexes of the vertices that are adjacent to $u$ with color $color$, based on the current
positions of the vertices. The algorithm is shown below:
...
$$
I \coloneqq \{i \in [n - 1]: \{x_i, y\} \in A(D)\} \text{ and } \bar{I} \coloneqq \{i \in [n - 1]: edge(y, x_{i+1}, cy) \in E(G)\}.
$$

Let's find the answer for the case $d^-_D(y) \geq \frac{n}{2}$. We have that 
$|I| + |\bar{I}|  \geq d^-_D(y) +  \frac{n}{2} $. 
Thus, from Dirichlet Principle, $I \cap \bar{I} \neq \emptyset$. In this case, we can build a cycle of length $n$ with the following crossing, removing the edge $x_i$: