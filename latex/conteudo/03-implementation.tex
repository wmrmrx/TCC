...
The complexity is in fact $O(n^2)$ because once we find valid edge $edge\_u$, we 
    ...
    \begin{algorithmic}
        \Function{Extend\_Cycle}{$G, C$}
            ...
            \For{$i \in [0, \dots, C.vertices.\text{size()} - 1]$}
                \State $edge_1 \gets \text{check\_edge}(G, u, C.vertices[i + 1], c1)$
                \State $edge_2 \gets \text{check\_edge}(G, u, C.vertices[i], c2)$
                ...
                \If{$edge_1 \neq \text{None} \text{ and } edge_2 \neq \text{None}$}
                    ...
                    \State \hspace{3.3em} $[edge_2, edge_1]$
