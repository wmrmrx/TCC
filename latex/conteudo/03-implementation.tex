...
\subsubsection{Case 2: \( l > \left \lceil \frac{n}{2} \right \rceil \)}

Let $cx$ be a color that is not on the path and 
$cy$ be the color of edge $e_{l}$. Remove the last edge and
vertex of the path and assign $x \leftarrow v_0$, $y \leftarrow v_{l}$.

We can first check if the edge $\{x, y\}$ belongs to $G_{cx}$ or
to $G_{cy}$. If it does, we just found a cycle of size $l$.
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Function{Extend\_Path\_Big}{$G, P$}
            ...
            \If{$edge \gets G.\text{check\_edge}(x, y, c) \neq \text{None}$}
...
All of this can be done in $O(n)$ time complexity, the time need
to find a color outside the path and create the object Cycle. We can now check if there exists a vertex $i$ outside the Path
such that $i$ is connects to the path on vertices $x$ and $y$
...
\begin{center}
    ...
    % Text Node
    \draw (109,131.4) node [anchor=north west][inner sep=0.75pt]    {$v_{0}$};
    % Text Node
    \draw (349,131.4) node [anchor=north west][inner sep=0.75pt]    {$v_{l}$};
    % Text Node
    \draw (179,131.4) node [anchor=north west][inner sep=0.75pt]    {$v_{1}$};
    % Text Node
    \draw (272,131.4) node [anchor=north west][inner sep=0.75pt]    {$v_{1}$};
    % Text Node
...
This can also be done in $O(n)$ time complexity, 
