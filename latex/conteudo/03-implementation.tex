...
$A(X)$ is the set of edges of object $X$.

The pseudocode is written using Python methods and conventions, such as array slicing, 
list appending, etc.
...
In this case, we start with a path \( P = \{x_0, e_0, \dots, x_{l-1}, e_{l-1}, x_{l}\} \) of length \( l \). The goal is to find either a path of length \( l+1 \) or a cycle of length \( l \) or \( l+1 \). To assist in this process, we define the following variables:
...
\( d(c, x_{l + 1}) \geq \left \lceil \frac{n}{2} \right \rceil \), from Dirac's condition, 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $last\_vertex \gets P.vertices.\text{back()}$
            ...
            \If{\textbf{not} $colors\_in\_path[color]$} \Comment{Search for an unused color}
                \For{$i \in [0, \dots, n-1]$}
                    \If{\textbf{not} $vertices\_in\_path[i]$} \Comment{Check vertices outside the path}
                        ...
                        \If{$edge \neq \text{None}$}
                            \State $vertices \gets P.vertices + [i]$
                            \State $edges \gets P.edges + [edge]$
...
We can check if the edge $\{x_1, x_{l}\}$ belongs to $E(G_{c_x})$ or
to $E(G_{c_y})$. If it does, we just found a cycle of size $l$. 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{Extend\_Path\_Big}{$G, P$}
            ...
            \State $c_y \gets \text{any color not in } colors\_in\_path$ \Comment{Find a color not in the path}
