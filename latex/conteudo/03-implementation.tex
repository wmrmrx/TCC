...
The total number of vertices is denoted by $n$, and the graph that is the union of $n$ graphs is represented by 
$G = G_1 \cup G_2 \cup \dots \cup G_n$, where each graph $G_i$ 
satisfies Dirac's condition. Moreover, we use the following notation:
...
\begin{itemize}
    \item Some variables used on a pseudocode may be defined on other parts of the same function;
    \item The variables on the images are the same as the ones defined in the proof and the code;
    \item Colors $c_x$ in the proof may be represented as $c(x)$ in the images.
\end{itemize}
...
a path of length $\ell+1$. In Figure \ref{fig:flowchart}, a $L$-Path is a path of length $L$ 
and a $L$-cycle is a cycle of length $L$.
...
We will assume that the algorithm is processing a graph of order $n \geq 6$. When $n < 6$ the result is known to hold
...
\begin{itemize}
    \item \texttt{colors\_in\_path}: an array of size \( n \) in which \texttt{colors\_in\_path[i]} is \texttt{True} if color \( i \) is used in the edges of the path.
    \item \texttt{vertices\_in\_path}: an array of size \( n \) in which \texttt{vertices\_in\_path[i]} is \texttt{True} if vertex \( i \) is included in the path.
\end{itemize}
...
and uses colors $c_x$ and $c_y$, as shown in Figure \ref{fig:path_vertex_outside}:
...
\begin{itemize}
    \item \texttt{colors\_in\_cycle}: an array of size \( n \) in which \texttt{colors\_in\_cycle[i]} is \texttt{True} if color \( i \) is used in the edges of the cycle.
    \item \texttt{vertices\_in\_cycle}: an array of size \( n \) in which \texttt{vertices\_in\_cycle[i]} is \texttt{True} if vertex \( i \) is included in the cycle.
\end{itemize}
...
Thus, we can build a path of length $\ell + 1$ by removing from the cycle an edge that is incident
to $w$ (see Figure \ref{fig:cycle_path_extension}):
...
$$
I_1 \coloneqq \left\{ i \in [0, \ell - 1]: \{u, x_{i + 1}\} \in A(G_{c_1}) \right\} \text{ and } I_2 \coloneqq \left\{ i \in [0, \ell - 1]: \{x_i, u\} \in A(G_{c_2}) \right\}.
$$

We have that $|I_1| + |I_2| = |N_{G_{c_1}}(u)| + |N_{G_{c_2}}(u)| > \ell$. Thus, 
by the Pigeonhole Principle, $I_1 \cap I_2 \neq \emptyset$. In this case, we can build a cycle of length $\ell + 1$
as indicated in Figure \ref{fig:cycle_cycle_extension}:
...
\begin{itemize}
    ...
    \item \texttt{find\_answer(G, y, cy, C, i)}. Let $G$ be the input graph collection, $C$ a cycle with
...
This case is slightly more complicated. It is the last iteration of the algorithm and will
...
\begin{itemize}
    \item \texttt{new\_color\_id}: an array of size \( n \) in which \texttt{new\_color\_id[i]} is the new label of color~\( i \),
    \item \texttt{vertex\_position\_on\_cycle}: an array of size \( n \) in which \texttt{vertex\_position\_on\_cycle[i]} is the position of vertex~\( i \) in the cycle.
    \item \texttt{color\_in\_position}: an array of size \( n \) in which \texttt{color\_in\_position[i]} is the color of the edge that connects the vertices~\( i \) and~\( i + 1 \) in the cycle.
\end{itemize}
...
$$
A(D) = \bigcup_{i \in \{0,..., n - 2\}} \left\{ \{x_i, z\} : z \neq x_{i + 1}, \{x_i, z\} \in A(G_{C.edges[i].color}) \right\}.
$$
...
Since $\delta(G_i) \geq \frac{n}{2}$ for all $i \in [0, n - 2]$, we have 
$d^+_{D}(x) \geq \frac{n}{2} - 1$ for every vertex~$x$ in the cycle. Thus,
$|A(D)| \geq (n-1)(\frac{n}{2} - 1)$. 

Now, let us define the following variables:

\begin{itemize}
    \item $in\_degree$: an array of size $n$, in which $in\_degree[i]$ is the in-degree of vertex $i$ in the digraph $D$.
    \item $incoming\_neighborhood$: an array of size $n$, in which $incoming\_neighborhood[i]$ is the list of vertices that have an edge entering vertex $i$ in the digraph $D$.
    \item $I \coloneqq \left\{i \in [n - 1]: \{x_i, y\} \in A(D)\right\}$.
    \item $\bar{I} \coloneqq \left\{i \in [n - 1]: \{y, x_{i+1}\} \in A(G_{cy})\right\}$.
\end{itemize}
...
Thus, by the Pigeonhole Principle, $I \cap \bar{I} \neq \emptyset$. In this case, we can build 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFunction
    \end{algorithmic}
    \label{alg:extend_cycle_case_2}
\end{algorithm}

The complexity of Algorithm~\ref{alg:extend_cycle_case_2} is $O(n^2)$, as we just need to iterate over $I$, 
check if an element in $I$ is in $\bar{I}$ and, once we find an element in $\bar{I}$, 
we call the function \texttt{Find\_Answer} that has complexity $O(n)$.

From now on, we assume that $d^-_D(y) < \frac{n}{2}$ and consider two cases:
...
To do this, we can just find a vertex with in-degree greater than $\frac{n}{2} - 1$ and rotate the cycle, 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFunction
    \end{algorithmic}
    \label{alg:extend_cycle_case_1}
\end{algorithm}

The time complexity of Algorithm~\ref{alg:extend_cycle_case_1} is $O(n)$, as we iterate over the vertices and
as soon as we find the desired vertex, we make $O(n)$ operations to rebuild the needed variables and then we break.

Let us define the following variables:

\begin{itemize}
    \item $I_0 \coloneqq \left\{i \in [n - 1]: \{x_i, y\} \in A(G_{c_0})\right\}$
    \item $I_{n-1} \coloneqq \left\{i \in [n - 1]: \{y, x_{i+1}\} \in A(G_{c_{n-1}})\right\}$
\end{itemize}

We have that $|I_0| + |I_{n-1}| \geq d(0, y) + d(n-1, y) \geq n$. Thus, 
by the Pigeonhole Principle, $I_0 \cap I_{n-1} \neq \emptyset$. Take $j \in I_0 \cap I_{n-1}$.
...
Let us construct a Hamiltonian path $P$ indicated in Figure~\ref{fig:cycle_n-1_build_path}:

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figuras/cycle_n-1_build_path.png}
    \caption{Building a Hamiltonian path $P$.}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFunction
    \end{algorithmic}
    \label{alg:extend_cycle_case_1_build_path}
\end{algorithm}

The time complexity of Algorithm~\ref{alg:extend_cycle_case_1_build_path} is $O(n^2)$, 
as we iterate over $I_0$, check if an element in $I_0$ is in $I_{n-1}$ and, if it is, we build the path $P$.
To build the path it takes $O(n)$ operations.

Let us now create the following variables:

\begin{itemize}
    \item $J_0 \coloneqq \left\{i \in [n-2]: edge(P\_vertices[0], P\_vertices[i + 1], removed\_color) \in E(G)\right\}$.
    \item $J_{n-1} \coloneqq \left\{i \in [n-2]: P\_vertices[i] \in incoming\_neighborhood[P\_vertices[n - 1]]\right\}$.
\end{itemize}


If $edge(P\_vertices[0], P\_vertices[-1], removed\_color)$ is in $G$, we can just 
close the cycle adding this edge. Let us assume that this is not the case. 
Thus, $|J_0| \geq \delta(G_{removed\_color}) \geq \frac{n}{2}$. Also, as $P\_vertices[n-2] \in \{x_{n-2}, y\}$, 
from the construction of $D$ we guarantee that $P\_vertices[n-2] \notin N^-_D(x_0)$
and consequently $|J_{n-1}| \geq \frac{n}{2} - \frac{1}{2}$.

By the Pigeonhole Principle, as $|J_0| + |J_{n-1}| \geq n$, we have that $|J_0 \cap J_{n-1}| \geq 2$. Thus,
there is at least one element in $J_0 \cap J_{n-1}$, say $k$, such that $P\_vertices[k] \neq y$.
We can then build the following Hamiltonian cycle:
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFunction
    \end{algorithmic}
    \label{alg:extend_cycle_case_1_build_cycle}
\end{algorithm}

The time complexity of Algorithm~\ref{alg:extend_cycle_case_1_build_cycle} is $O(n^2)$, 
as we iterate over $J_0$ and check if an element of $J_0$ is in $J_{n-1}$. If yes, 
we build the cycle and return it, in $O(n)$ operations.
Therefore, when there exists~$x_i$ such that $d^-_D(x_i) > \frac{n}{2} - 1$, we have a total 
complexity of $O(n^2)$.

Now, let us analyze the case in which every $x_i$ satisfies $d^-_D(x_i) \leq \frac{n}{2} - 1$.
Define

\begin{equation}
    \mathcal{J} \coloneqq \left\{i \in [n-1]: d^-_D(x_i) = \left\lfloor \frac{n}{2} - 1 \right\rfloor\right\}.
...
\begin{equation}
    |A(D - y)| \geq \left(n - 1\right) \left(\frac{n}{2} - 1\right) - \frac{n}{2} + 1 > \left(n - 1\right) \left(\frac{n}{2} - \frac{3}{2}\right).
...
\begin{equation}
    |\mathcal{J}| \left\lfloor \frac{n}{2} - 1 \right\rfloor + \left(n - 1 - |\mathcal{J}|\right) \left\lfloor \frac{n}{2} - 2 \right\rfloor \geq |A(D - y)| > \left(n - 1\right) \frac{n}{2} - \frac{3}{2}.
\end{equation}

Therefore, $|\mathcal{J}| \geq \frac{n}{2} > \frac{n - 1}{2}$. Defining 
$\mathcal{J}' \coloneqq \left\{i \in [n - 1]: edge(x_{i+1}, y, c_y) \in A(G)\right\}$, we have that 
$|\mathcal{J}'| + |\mathcal{J}| \geq n$. By the Pigeonhole Principle, there exists 
...
\begin{figure}[H]
    ...
    \caption{Empirical time complexity analysis. The blue points represent the average execution times 
    for different values of $n$, while the red curve shows the theoretical $O(n^3)$ bound.}
...
\begin{itemize}
    ...
    \item For each graph of order $n$ ranging from $n=6$ to $n=100$, we ran multiple test iterations;
    \item Each test verified that the algorithm successfully found a rainbow Hamiltonian cycle.
\end{itemize}
...
This test uses carefully constructed input graphs to verify the algorithm's behavior in this specific scenario.
